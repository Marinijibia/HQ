...
function ResetPasswordForm() {
  ...
  return (
    <Card className="border border-white/10 bg-[#0A0B10]/85 backdrop-blur-3xl shadow-[0_0_50px_rgba(168,85,247,0.15)] text-foreground p-4 rounded-3xl relative overflow-hidden transition-all duration-300">
      ...
      {success ? (
        <CardHeader className="text-center space-y-4 py-10">
          ...
          <CardTitle className="text-2xl font-black text-white">Password Reset Complete!</CardTitle>
        ...
        <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  required
                  className="bg-black/50 border-white/10 text-white h-11 text-xs focus-visible:ring-purple-500 rounded-xl placeholder:text-slate-600"
                />
              </div>
          ...
          <CardFooter className="flex justify-center border-t border-white/10 pt-4 mt-2">
            <Link href="/login" className="text-xs text-slate-400 hover:text-white flex items-center gap-1.5 font-semibold">
              <ArrowLeft className="h-3.5 w-3.5 text-purple-400" /> Cancel & Return to Sign In
            </Link>
          </CardFooter>
        </>
      )}
...
export default function ResetPasswordPage() {
  return (
    <div className="min-h-screen bg-[#050508] text-foreground flex flex-col justify-between font-sans relative overflow-hidden select-none animate-in fade-in duration-500">
      ...
      <div className="absolute top-[-20%] left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-radial from-purple-600/15 via-pink-600/10 to-transparent blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] bg-radial from-cyan-600/10 via-blue-600/5 to-transparent blur-[140px] pointer-events-none" />
      ...
      <header className="flex h-20 items-center justify-between border-b border-white/10 px-6 sm:px-12 bg-[#0A0B10]/60 backdrop-blur-2xl relative z-10">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-slate-400 font-medium tracking-wide">Password Update Gateway</span>
          </div>
        </div>

        <Badge variant="outline" className="border-purple-500/30 bg-purple-500/10 text-purple-300 text-[11px] font-bold px-3 py-1 rounded-full flex items-center gap-1.5">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 relative z-10 my-8">
        <div className="w-full max-w-md">
          <React.Suspense
            fallback={
              <Card className="border border-white/10 bg-[#0A0B10]/85 p-8 text-center rounded-3xl">
                <Loader2 className="h-8 w-8 animate-spin text-purple-400 mx-auto mb-3" />
                <p className="text-xs text-slate-400 font-semibold">Loading Reset Gateway...</p>
              </Card>
            }
          >
            <ResetPasswordForm />
