...
export default function MissionTimelinePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchMission = React.useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/missions/${resolvedParams.id}`, { headers });
      if (res.ok) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background select-none">
        <div className="flex flex-col items-center space-y-3">
          <Loader2 className="h-8 w-8 text-hq-blue animate-spin" />
          <p className="text-xs text-foreground/50">Fetching executive briefings log...</p>
        </div>
      </div>
    );
  }

  // Fallback to demo steps only if the backend returns no tasks
  const steps: TimelineStep[] =
    mission?.tasks && mission.tasks.length > 0
      ? mission.tasks.map((t: any) => {
          ...
          return {
            stageName: t.name,
            assignedDirector: t.executive
              ? `${t.executive.name} (${t.executive.title})`
              : 'Autonomous Agent',
      ...
      : [
          {
            stageName: 'CEO Strategic Assessment',
          ...
          {
            stageName: 'Chief of Staff WBS DAG Plan',
    ...
    <div className="space-y-8 select-none">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-[#1A1A1E] dark:text-white">Mission Timeline</h1>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <Card className="border border-hq-graphite/40 bg-hq-graphite/20">
            <CardHeader className="flex flex-row items-start justify-between pb-4">
              <div>
                <CardTitle className="text-base text-[#1A1A1E] dark:text-white font-extrabold">
                  {mission?.objective || 'Q3 Petroleum Logistics Outreach Strategy'}
                </CardTitle>
                ...
                {steps.map((step, idx) => {
                  ...
                  return (
                    <div key={idx} className="relative">
                      {/* Left Dot Node */}
                      <span className="absolute -left-[31px] top-1 flex h-4 w-4 items-center justify-center rounded-full bg-background border border-card-border">
                        {isCompleted && <CheckCircle2 className="h-4 w-4 text-hq-cyan bg-background rounded-full" />}
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between">
                          <h4
                            className={`text-sm font-bold ${isRunning ? 'text-hq-blue' : isFailed ? 'text-red-400' : 'text-foreground'}`}
                          >
                            {step.stageName}
                          </h4>
                          {step.timestamp && (
                            <span className="text-[10px] text-foreground/45 font-semibold">{step.timestamp}</span>
                          )}
                        </div>
                        ...
                        <p
                          className={`text-xs mt-1 leading-relaxed ${isFailed ? 'text-red-400/80 font-medium bg-red-500/5 border border-red-500/20 rounded p-2' : 'text-foreground/50 font-semibold'}`}
                        >
                          {step.message}
                        </p>
                      </div>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-extrabold text-[#1A1A1E] dark:text-white">Oversight Operations</CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  className="flex-1 flex items-center justify-center gap-1 text-xs font-bold border-red-500/20 text-red-400 hover:bg-red-500/10"
                >
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-1.5 font-extrabold text-[#1A1A1E] dark:text-white">
