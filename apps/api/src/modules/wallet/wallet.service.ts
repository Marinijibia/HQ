...
export class WalletService {
  ...
  async getWallet(companyId: string) {
    ...
    try {
      ...
      // Raw SQL fallback with parameterized inputs for database resilience
      const rawRes = await this.prisma
        .$queryRawUnsafe(
          `SELECT * FROM organization_wallets WHERE company_id::text = $1 LIMIT 1`,
          companyId,
        )
        .catch(() => []);
      const rows = Array.isArray(rawRes) ? rawRes : [];
      ...
      await this.prisma
        .$executeRawUnsafe(
          `INSERT INTO organization_wallets (id, company_id, balance_usd, currency, status, created_at, updated_at)
           VALUES ($1, $2, 100.0, 'USD', 'ACTIVE', NOW(), NOW())
    ...
    try {
      ...
      await this.prisma
        .$executeRawUnsafe(
          ...
          amountUsd,
          'COMPLETED',
          description || 'Fiat Balance Deposit',
        )
        .catch(() => {});
    }
    ...
    const defaultRoles = [
      {
        ...
        monthlyLimit: 1000.0,
        singleTxLimit: 100.0,
        requireApprovalAbove: 100.0,
      },
      {
        roleKey: 'CEO',
        monthlyLimit: 2000.0,
        singleTxLimit: 250.0,
        requireApprovalAbove: 250.0,
      },
      {
        roleKey: 'CTO',
        monthlyLimit: 1000.0,
      ...
      },
      {
        ...
        monthlyLimit: 1500.0,
        singleTxLimit: 200.0,
        requireApprovalAbove: 200.0,
      },
      {
        roleKey: 'CMO',
        monthlyLimit: 800.0,
        singleTxLimit: 75.0,
        requireApprovalAbove: 75.0,
      },
    ];
   ...
   */
  async executeAutonomousAgentPayment(
    companyId: string,
    dto: ExecuteAgentPaymentDto,
  ) {
    ...
    if (!dto.vendorAddress)
      throw new BadRequestException('Vendor destination address is required');
    ...
    try {
      await this.prisma.agentAllowance
        .update({
          ...
          data: {
            currentMonthSpent: agentPolicy.currentMonthSpent + amountUsd,
          },
        })
        .catch(() => {});
    } catch {
    ...
    try {
      ...
      await this.prisma.organizationWallet
        .update({
          where: { companyId },
      ...
      this.logger.error(
        `[Wallet] On-chain transfer failed. Refunded balance to ${companyId}: ${circleErr.message}`,
      );
      throw new BadRequestException(
        `Circle USDC payment dispatch failed: ${circleErr.message}. Funds refunded to HQ Wallet.`,
      );
    }
  ...
  async getTransactions(companyId: string) {
    try {
      ...
      const rawRes = await this.prisma
        .$queryRawUnsafe(
          `SELECT * FROM wallet_transactions WHERE company_id::text = $1 ORDER BY created_at DESC LIMIT 50`,
          companyId,
        )
        .catch(() => []);
      const rows = Array.isArray(rawRes) ? rawRes : [];
